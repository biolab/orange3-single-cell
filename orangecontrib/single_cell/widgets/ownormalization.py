...
class OWNormalization(widget.OWWidget):
    ...
    class Warning(widget.OWWidget.Warning):
        no_discrete_attributes = widget.Msg("No discrete variables in input data.")
    ...
    def __init__(self):
        ...
        combo_attrs = self.combo_attrs = gui.comboBox(
            ...
            box="Equalize read depth on column",
        ...
        box = gui.widgetBox(self.controlArea, 'Steps and parameters')
        gui.checkBox(gui.vBox(box), self, "equalize_lib", "Equalize library read depth",
                     callback=self.on_changed)
        gui.checkBox(gui.vBox(box), self, "normalize_cells", "Normalize cell profiles",
                     callback=self.on_changed)
        gui.spin(box, self, "log_base", 2.0, 10.0, label="Log(1 + x) transform, base: ",
